...
from config.constants import FEATURE_DIMS, FEATURE_KEYS_CT, FEATURE_KEYS_nonCT
    ...
    def forward(self, event):
        ...
        for key in self.img_feature_linears:
            ...
            features = features + scanid_embd[None, :, None, None, None, :] + pos_embd[None, :, None, None, None,
                                                                              :]  # (1, n_patches, 8, 8, 8, dim)
            _, n, d, h, w, _ = features.shape
            features = rearrange(features, '() n d h w c -> (n d h w) c')  # (1, n_patches * 8 * 8 * 8, dim)
...
class GatedVideoEventFormer(nn.Module):
    ...
    def check_no_feature(self, event):
    ...
    def forward(self, event):
        if self.check_no_feature(event):
            raise ValueError(f'Event {event} on {event.get("date")} has no image features.')
        ...
        # list_of_tensors = [torch.randn(1, 256, 16, 16, 256, device=tensor.device)] * 1  # a test with dummy var
