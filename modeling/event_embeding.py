...
"""
This file defines a series of classes for event embedding. We need to model the embedding process by class because
there may be learnable parameters in the embedding process.

Image feature types:
1. CT: SAMMed3D (num_patches, 384, 4, 4, 4), universal-clipdriven (num_patches, 384, 3, 3, 3), fmcib (num_patches, 4096)
2. US, ES, CR, MR: SAMMed2D -- (1, 256, n_slices, 4, 4) * num_scans (normally = 1)
"""
...
class AvgEventEmbedding(AbstractEventEmbedding):
    ...
    def embed_img_event(self, event):
        ...
        for key in self.img_feature_keys:
            ...
            for event_feature in event['img_features'][key]:
                ...
                if len(event_feature.shape) == 5:  # 3D feature map
                    ff += torch.mean(event_feature, dim=[0, 2, 3, 4])  # (num, dim, 4, 4, 4) -> (dim,)
                else:
                    ff += torch.mean(event_feature, dim=0)  # FMCIB: (num, dim) -> (dim,)
            ff /= len(event['img_features'][key])
            ff = getattr(self, f'{key}_linear')(ff)  # (dim,)
            features.append(ff)
        if features:
            # features = torch.stack(features, dim=0)  # (num_img_features_keys, dim)
            features = sum(features) / len(features)  # (dim,)
            features = features[None]  # (1, dim)
        else:
            print(f'Event {event["pacs"]} {event["content"]} {event["uid"]} {event["event_type"]} on {event.get("date")} has [empty] image features.')
    ...
    def embed_text_event(self, event):
        ...
        if 'txt_features' not in event or not event['txt_features']:
            print(f'Event {event["event_type"]} on {event.get("date")} has no text features.')
            return torch.zeros(1, self.hidden_size, device=self.device, requires_grad=True)
    ...
    def embed_lab_event(self, event):
        if 'lab_features' not in event:
            print(f'Event {event["event_type"]} on {event.get("date")} has no lab features.')
            return torch.zeros(1, self.hidden_size, device=self.device, requires_grad=True)
        f = event['lab_features']
        f = self.lab_linear(f.reshape(1, FEATURE_DIMS['lab']))  # (1, d)
        return f
